...
Let us revisit the basic sinusoid from [Chapter 3](../ch03/index.md), taking unit amplitude and zero initial phase for simplicity:
...
:::{tip}
As always, if you feel rusty with angular frequency, revisit [Chapter 3](../ch03/index.md). We work in angular frequency $\omega$ here to keep the expressions compact.
:::
...
:::{note}
Do not be alarmed by the integral sign. As we noted in [Chapter 5](../ch05/index.md), working out closed-form integrals is not a focus of this book. Read the integral at a high level: it simply **sums up how much phase has elapsed** up to time $t$. When $\omega(\tau) = \omega$ is constant, the area under a flat line from $0$ to $t$ is just $\omega t$, recovering the familiar $\sin(\omega t)$. That equivalence is exactly why we "got away with" the simple form for constant-frequency tones.
:::
